...
export const centerTextPlugin = {
  ...
  beforeDraw(chart) {
    ...
    const totalvalue = chart.data.datasets[0].data[1] ?? 0;
    // let dataset = chart.data.datasets[0];
    // eslint-disable-next-line no-underscore-dangle
    // let meta = dataset._meta[Object.keys(dataset._meta)[0]];
    // let { total } = meta;
    // console.log(dataset);
    let totalsumvalue = activeValue + totalvalue;
    let activePercentage = parseFloat(activeValue.toFixed(0));
    totalsumvalue = parseFloat(totalsumvalue.toFixed(0));
    activePercentage = chart.legend.legendItems[0].hidden
      ? 0
      : activePercentage;
    activePercentage = `${activePercentage} / ${totalsumvalue}`;
